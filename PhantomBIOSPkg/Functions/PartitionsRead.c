...
EFI_STATUS ReadRawDataRead(EFI_BLOCK_IO_PROTOCOL *BlockIo, UINT64 Lba, UINTN BufferSize, CHAR16 *FilePath) {
    EFI_STATUS Status;
    VOID *Buffer;
...
EFI_STATUS ListDirRead(EFI_FILE_PROTOCOL *Dir, CHAR16 ***FileList, UINTN *FileCount) {
    EFI_STATUS Status;
...
EFI_STATUS ListPartitionsRead() {
    ...
    for (i = 0; i < HandleCount; i++) {
        ...
        if (!EFI_ERROR(Status)) {
            ...
            Status = ListDirRead(Root, &FileList, &FileCount);
            if (EFI_ERROR(Status)) {
                Root->Close(Root);
                continue;
            }
    ...
    for (i = 0; i < UnsupportedCount; i++) {
        ...
        Status = ReadRawDataRead(BlockIo, 0, 4096, FilePath);
        if (EFI_ERROR(Status)) {
